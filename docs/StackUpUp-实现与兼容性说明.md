...
另外，脚本层当前也是按 `registryName` / 类 / 数值属性匹配，没有看到专门的 metadata token；所以它天然是 **Item 级**，不是 **meta 级**。
...
1. **增加 metadata 维度的规则语法**  
   例如支持 `id + meta`、`damage` 或 NBT 条件，这样 GT 这种物品才能按子类型单独配置。

2. **把规则存储从 Item 改为 Item+meta**  
   这会影响脚本、重载、序列化和部分 patch 逻辑，是最直接但也最重的改法。
