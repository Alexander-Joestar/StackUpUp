...
本节记录 11.13/0.7.1 → 11.17/0.7.2 的成对升级实施与实测。升级本身已完成并通过编译/测试/refmap 验证；**但带真实第三方模组的服务端矩阵仍 FAIL**，本次升级没有消除任务上下文所报的 ASM 5.2 崩溃。以下只是实施与失败记录，不构成验收通过。
...
- **验证 1（PASS，refmap 未丢）：** `JAVA_HOME=/c/Users/25761/.jdks/jbr-25.0.4 ./gradlew --no-daemon compileJava` → BUILD SUCCESSFUL（3m41s）；javac 注记 `SpongePowered MIXIN Annotation Processor Version=0.8.7` 与 `Writing refmap to build/tmp/mixins/mixins.StackUpUp.refmap.json`；产物 11906 B（mtime 2026-09-15 20:02:24），`processResources` 后 `build/resources/main/mixins.StackUpUp.refmap.json` 同尺寸 11906 B。
- **验证 2（PASS）：** `./gradlew --no-daemon test` → BUILD SUCCESSFUL（53s）。
- **验证 3（FAIL，如实记录）：** `runServerAutoTestMatrix` 未通过。工作副本的 `local-dev-mods/` 为 gitignored 且缺失，首次运行（20:04–20:05）只装载 4/8 个 mod（`Finished gathering 4 mods`、`successfully loaded 8 mods`）、AE2 未在场，`run/logs/autotest-report.txt`（时间戳 2026-09-15 20:05:06）记 `最终状态: PASS`——该次**未覆盖目标场景**，不能作为“越过 ae2.mixins.json”的证据。把主检出 `local-dev-mods/*.jar`（10 个，只读复制、不进版本库）放入工作副本后重跑，**完整复现 M17 的原始崩溃**（2026-09-15 20:09:28）：`org.spongepowered.asm.launch.MixinInitialisationError: Error initialising mixin config ae2.mixins.json` → `java.lang.IllegalArgumentException: The requested compatibility level JAVA_11 could not be set. Level is not supported by the active JRE or ASM version (Java 1.8, ASM 5.2 (ASM5))`，BUILD FAILED。AE2 配置实测 `ae2.mixins.json` 为 `compatibilityLevel: JAVA_11`、`minVersion: 0.8.3`。结论：**成对升级本身不修复该崩溃。**
- **根因更正（推翻任务 Context 的前提）：** 运行期 classpath 上唯一的 ASM 是 `asm-debug-all:5.2`。对 `runServer` 任务全部 74 个 classpath jar 逐一扫描 `org/objectweb/asm/Opcodes`，只有 `asm-debug-all-5.2.jar` 命中；cleanmix 0.7.2 携带的 asm 9.8 只存在于 **annotationProcessor** 配置（`api(mixin)` 与 `annotationProcessor(mixin)` 均为 `isTransitive=false`，cleanmix 又只挂 AP），因此 9.8 根本不进 `runtimeClasspath`。`dependencyInsight --configuration runtimeClasspath --dependency asm-debug-all` 显示来源为 `runtimeClasspath`；进一步实测该 5.2 来自 RFG 自身：RFG 2.0.2 源码 `MCPTasks.java:1023-1056` 对 MC 1.12.2 显式 `deps.add(PATCHED_MC_CFG, "org.ow2.asm:asm-debug-all:5.2")`（配置名 `patchedMinecraft`），并经 `MCPTasks.java:916` 的 `runtimeClasspath.extendsFrom(patchedMinecraft)` 进入运行 classpath，**不是** `build-logic/convention/src/main/kotlin/localDevMods.gradle.kts:88`。Mixin 的 ASM 探测（`org.spongepowered.asm.util.asm.ASM` 读 `org/objectweb/asm/Opcodes`）因此只看到 5.2。
- **任务指定回退方案实测（均已还原，未留在分支）：** ① 仅从 `runtimeClasspath` 排除 `asm-debug-all` → 崩溃不变（`patchedMinecraft` 仍带入 5.2，探针实测运行 classpath 仍只有 5.2）；② 同时从 `runtimeClasspath` + `patchedMinecraft` 排除 → `:compilePatchedMcJava` 以 `java.lang.NullPointerException`（Gradle `ConstantsTreeVisitor.getBinaryClassName`）FAILED，构建直接破坏；③ ② 基础上再加 `asm`/`asm-tree`/`asm-commons`/`asm-util`/`asm-analysis` 9.8 运行依赖 → 同样 `compilePatchedMcJava` NPE（且违反任务「不新增依赖」）；④ `api(mixin)` 改 `isTransitive=true` → 崩溃不变（ASM 5.2 仍先命中）。结论：`exclude module: 'asm-debug-all'` 这条上游做法在本项目 RFG 2.0.2 布局下不可行——它要么不生效，要么打掉 RFG 编译 patched MC 所需的 ASM。要真正越过 `JAVA_11`，需让 ASM ≥ 7 成为运行期 ASM 提供者（引入运行期 asm 9.x，或调整 RFG `patchedMinecraft` 的 ASM 选择），均超出本任务「不改依赖」的范围，属需要协调者决策的后续工作。
- **剩余风险与未覆盖：** 带真实第三方 mod 的服务端矩阵仍 FAIL（AE2 等 `compatibilityLevel: JAVA_11` 配置不可装载）；`runClient` 未跑；工作副本内的 `local-dev-mods/` 为诊断用只读复制（gitignored，不随分支进版本库），因此**评审员在其他检出上跑矩阵会再次得到“无 mod 的 PASS”**，须自行放入 jar 才能复现失败。独立复核未做，本记录不宣布升级验收通过。
