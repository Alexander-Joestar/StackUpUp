...
- `src/main/kotlin/io/alexjoest/stackupup/bootstrap/StackUpUpMixinConnector.kt:12-37,41-69` 实现 `IMixinConnector`，执行 early/late 配置入队与决策：early 处理冲突，late 处理未知配置和 mod presence；`MixinToggles` 是已移除的历史纯代码假开关，当前单个已登记配置的禁用入口是 MixinBooter `config/mixinbooter.cfg` 的 `general.blacklistedConfigs`。已删除的 `MixinConfigValidator` 双向配置校验不属于当前 connector。其公共无参 `class` 形态是当前装载约束。
...
- **实施变更（逐文件）：**
  - 新建 `src/main/kotlin/io/alexjoest/stackupup/bootstrap/StackUpUpMixinConnector.kt`：`class StackUpUpMixinConnector : IMixinConnector`。**当时中间版本记录**：`connect()` 曾在 early `Mixins.addConfiguration` 前调用 `MixinConfigValidator.requireCoreConfigValid`，late 曾调用 `validateConfigs`/`logProblems`；这些自定义 validator 调用及其双向校验后来由提交 `30a5976` 删除。**当前事实**：early 由 `connectEarly` 依据 `StackUpUpCore.ensureConflictState()` 和 `shouldQueueEarly` 决定是否 `Mixins.addConfiguration`（当前文件 `:46-55`、`:179-190`）；late 由 `connectLate` 遍历当前 `modules`，`shouldQueue` 对未知配置和 mod 缺失分别作决策/INFO 或 ERROR 记录（`:58-67`、`:196-215`），不保留 validator；历史 `MixinToggles` 已移除，单个配置的禁用入口是 `config/mixinbooter.cfg` 的 `general.blacklistedConfigs`。
