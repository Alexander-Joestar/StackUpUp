...
JSONL 守恒报告均为历史基线，不得写成当前实现。T12 方案 A 已归档且不再作为 T13 依赖；T8.0 为 PARTIAL，T8.2/T13/T14.7
为 BLOCKED，T14.1–T14.6 为 PARTIAL，T16 未完成。
...
本节只收敛当前 Mixin 生态版本边界、注入器选择和参考仓库的可迁移范围；上游资料只作来源与架构佐证，当前实现以 11.13/0.7.1/connector 的源码、产物和运行证据为准，不改变前述容量事实。

### 8.1 已决策：当前构建事实与历史/上游资料分层

- **当前构建事实：** `gradle/libs.versions.toml:20-23,29-31` 为 MixinBooter `11.13`、CleanMix `0.7.1`；`build.gradle.kts:49-66` 将 CleanMix 作为编译期 annotation processor 挂载。当前注册入口是 manifest `MixinConnector` 指向 `StackUpUpMixinConnector`，不是旧 loader。
...
- **历史基线：** MixinBooter 10.7、`IEarlyMixinLoader`/`ILateMixinLoader`、`MixinTweaker` 和外部 LlamaLad7 MixinExtras 0.5.0 只用于版本对照。旧事实不得覆盖当前 11.13/0.7.1/connector。
...
- 11.13 迁移、connector、CleanMix AP/refmap 和 provider 证据已记录于 §8.6.1、§8.8；T14.7 准入仍未通过，不得把局部验证写成发布通过。
...
- [CleanMix 官方仓库](https://github.com/CleanroomMC/CleanMix) 是 Mixin 核心转换、classloading service、注入和 AP/refmap 来源；当前项目实际使用版本为 `0.7.1`，以配置、jar/POM 和运行证据为准。
- [SpongePowered/Mixin 官方仓库](https://github.com/SpongePowered/Mixin) 是上游注入器、Accessor/Shadow、AP 和混淆基础来源；不能把上游发行版本直接当作当前运行时版本。
- [CleanroomMC/MixinExtras 官方仓库](https://github.com/CleanroomMC/MixinExtras) 是当前 11.13 内嵌 fork 的来源边界；其 DeepWiki wiki 未索引，官方页面和实际 jar/POM 为一等证据。当前 provider 为 0.5.5，同包名无 relocation。
...
- **历史决策：** T9-R1/T9-R3 将 MixinBooter 11 设为计划内必选迁移，并以 T14.7 作为准入门；当时的 10.7/旧 loader 记录只代表迁移前基线。
- **当前结果：** 11.13 已进入构建，CleanMix `0.7.1` 作为编译期 AP；旧 `IEarlyMixinLoader`/`ILateMixinLoader` 注册路径已由 `IMixinConnector` 取代。迁移的 refmap/AP、dev connector 装载和服务端结果见 §8.6.1、§8.8，但 T14.7 仍为 BLOCKED/PARTIAL。
- 任何生产混淆、第三方写入、高风险 Mixin 或未独立复核项目仍只能写 `UNKNOWN`/`无源码不可判定`，不得以 11.13 已装载推出发布准入通过。
- **T14 状态：** [`T14 Mixin 生态与注入重构`](%E9%87%8D%E6%9E%84%E4%BB%BB%E5%8A%A1%E6%B8%85%E5%8D%95.md)；T14 及 T14.0–T14.7 的正式范围、验收与 DAG 以该清单当前内容为准（当前收口版已压缩为状态表、验收卡与 DAG，不再按节号引用）。T14.1–T14.6 当前为 PARTIAL，T14.7 为 BLOCKED/PARTIAL；11.13 迁移已实施，但迁移实施不等于准入通过。缺 jar、未运行生命周期验证或其他关键证据缺失时继续保留 **无源码不可判定**或 `UNKNOWN`，不得写成通过。正式定义前的建议/UNKNOWN 证据及其对照记录仍保留于 [`T14 对照与验收门`](%E5%80%9F%E9%89%B4%E4%BB%93%E5%BA%93%E4%B8%8E%E9%87%8D%E6%9E%84%E5%AF%B9%E7%85%A7.md)「5.1 T14：Mixin 生态与注入重构」及其子节，仅作历史/对照证据，不替代现行 T14 范围。
...
- **历史迁移顺序：** ① 将 `gradle/libs.versions.toml:20` 的 MixinBooter 版本从 10.7 升至 11.13；② 删除外部 MixinExtras 0.5.0 声明，改由 11.13 内嵌 fork 提供同包名 API；③ 编译、测试、spotless 与服务端矩阵验证；④ 复核 T14.7 准入门。实际执行另发现 refmap AP 回归，并已通过 `annotationProcessor(libs.cleanmix)` 修复，最终证据见 §8.6.1。
- **历史回滚边界：** 版本值和外部 MixinExtras 声明原可分别回退；此处仅记录当时的 jj 工作区回滚边界，不构成当前操作建议，也未执行任何回滚或清理。
- **历史触发与停止条件：** 原计划要求 T14.7 通过后才实施迁移，并在 provider、注册、refmap、classloader、容量证据或生命周期验证未闭合时停止。迁移已按授权实施，但 refmap 缺失曾实际命中停止条件，随后完成 AP 修复；T14.7 仍 BLOCKED/PARTIAL，故当前不得写成发布准入通过。
...
- **DeepWiki 与来源访问失败记录：** 本会话未注入 DeepWiki MCP 工具，降级为 DeepWiki 网站页面与官方 GitHub。`https://deepwiki.com/CleanroomMC/MixinBooter` 页面版本表止于 10.0，不能作为 11.x 注册事实；`https://deepwiki.com/CleanroomMC/CleanMix` 首次提取无文本、重试成功。GitHub API tree 请求 403 rate limit，降级 raw 文件直取成功。上述来源仅作架构/职责佐证，版本与运行结论以当前 11.13/0.7.1 的 POM、产物和 §8.6.1 实测为准。

#### 8.6.1 迁移实施记录（2026-08-09，历史中间阶段；T9-R3 授权的独立迁移实现任务实际执行）

本段是 MixinBooter 11 版本/AP 迁移的历史中间阶段记录（2026-08-09 05:37–05:48 实测），当时仍以旧 loader 验证版本升级，随后由 §8.8 迁移到 `IMixinConnector`。只记录当时执行结果与新发现，不改变 8.6 的决策、回滚边界与停止条件；迁移是否达到验收由 T14.7 准入门复核判定，实施任务不自宣通过。
...
本节是 T14.0「来源与版本账本复核」在迁移实施前的窄复核产出：只交叉核对当时的 T9 账本、构建配置、本地产物、DeepWiki/官方来源，不改依赖、不改 build 文件、不改 Mixin。它保留 10.7 与旧 loader 的历史证据，不代表当前构建；当前 11.13/0.7.1/`IMixinConnector` 及后续收口见 §8.6.1、§8.8。复核方式为 DeepWiki MCP（https://mcp.deepwiki.com/mcp，HTTP JSON-RPC/SSE）、官方 Maven（maven.cleanroommc.com / Maven Central）、官方 GitHub（页面/raw，GitHub REST API 被限流）与本机 gradle 缓存、build/ 产物交叉核对。

**T9 账本历史复核结论：** T9-R1/T9-R3 的版本决定、坐标、发布事实、回滚边界与当时的 4 项 UNKNOWN 与本复核逐条一致；以下仅保留当时的证据精度与 2 项新 UNKNOWN。迁移前构建触点复核：`gradle/libs.versions.toml:20` `mixinbooter = "10.7"`、`:21` `mixinextrasCommon = "0.5.0"`、`:29` `io.github.llamalad7:mixinextras-common`；`build.gradle.kts:53-54` `modUtils.enableMixins("zone.rong:mixinbooter:${...}")` 字符串插值、`:57-58` `compileOnly`/`annotationProcessor(libs.mixinextrasCommon)`、`:59-62` AP 追加 `asmDebugAll`/`guava`/`gson`/mixin（与 10.7 tag 上游 AP 集合一致）。这些触点已由当前实现更新，勿据此判断现状。
...
**历史 UNKNOWN 账本逐条（2026-08-08 快照；当前收口见 §8.6.1、§8.8）：**
...
- ⑥ **11.13 运行时 ASM 版本并存：仍 UNKNOWN。** 11.13/cleanmix-0.7.1 不内嵌 ASM，POM 分别带 asm-debug-all 5.2 与 asm 9.8；本次编译和 dev 服务端运行无异常，但转换链实际选用的 ASM 套数未单独审计。

**无源码不可判定与迁移行为边界：** 当前 11.13/cleanmix-0.7.1/Cleanroom MixinExtras 0.5.5 已有 dev/SRG 编译和服务端加载证据，但第三方容量写入路径、生产混淆环境的 connector/refmap 消费及高风险 Mixin 行为仍按既有证据缺口处理，第三方 jar 缺源码处为**无源码不可判定**。T14.7 仍 BLOCKED/PARTIAL，不能把迁移实施记录写成发布准入通过。第三方容量写入目标缺口维持既有台账（`compatibility-decision-record.md:156-163`、`mixin-生态与注入最佳实践.md:97-101`），本复核不重列。DeepWiki 全部查询只作架构/职责佐证，不作版本化运行事实；GitHub API 限流与 DeepWiki 未索引均已按上表逐条记录，未把失败项写成通过。
