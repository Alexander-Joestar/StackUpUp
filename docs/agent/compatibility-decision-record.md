...
- **剩余风险与未覆盖：** `runClient` 未跑（任务只要求服务端矩阵）；工作副本内的 `local-dev-mods/` 为 gitignored 的本地诊断复制，不随分支进版本库，因此**评审员在其他检出上跑矩阵时须自行放入正确 1.12.2 的 mod jar**（尤其 AE2 必须是 `ae2-uel-v0.56.7.jar` 一类 `mcmod.info`/1.12.2 产物，不能用 1.19 的 `appliedenergistics2-forge-*`，否则会得到上文的 JAVA_11 FAIL）；生产混淆 notch 环境与 `runClient` 路径未实测。独立复核未做，本记录不宣布升级验收通过。
