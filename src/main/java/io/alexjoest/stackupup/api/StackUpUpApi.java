...
/**
 * 对外公开门面：供 CraftTweaker 等外部脚本读取动态堆叠上限与规则状态。
 *
...
public final class StackUpUpApi {
    ...
    /** 写入状态，返回底层状态文件是否真的被改写；存储不可用时返回 false。 */
    public static boolean setState(String name, boolean value) {
        Boolean changed = STATE_SERVICE.setState(name, value);
