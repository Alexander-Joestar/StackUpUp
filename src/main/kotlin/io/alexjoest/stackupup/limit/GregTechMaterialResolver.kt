...
object GregTechMaterialResolver {
    ...
    private fun isGregTechLoaded(): Boolean = runCatching { Loader.isModLoaded(GREGTECH_MOD_ID) }.getOrDefault(false)
    ...
    internal fun resolveMaterialForTesting(stack: ItemStack, gregTechLoaded: Boolean): String = resolveMaterial(stack, gregTechLoaded)
    ...
    private data class ReflectionHandles(private val primary: PrimaryHandles?, private val fallback: FallbackHandles?) {
        fun resolve(stack: ItemStack): String = primary?.resolve(stack).orEmpty().ifEmpty { fallback?.resolve(stack).orEmpty() }
    }

    private data class PrimaryHandles(private val getMaterial: Method) {
        fun resolve(stack: ItemStack): String = runCatching {
            resolveMaterialStack(getMaterial.invoke(null, stack))
        }.getOrDefault("")

        companion object {
            fun create(): PrimaryHandles? = runCatching {
                PrimaryHandles(
    ...
    private data class FallbackHandles(private val metaItemClass: Class<*>, private val getItem: Method) {
        fun resolve(stack: ItemStack): String = runCatching {
            val item = stack.item
        ...
        companion object {
            fun create(): FallbackHandles? = runCatching {
    ...
    private fun materialNameFromValueItem(valueItem: Any): String = materialObject(valueItem)?.let { materialName(it) }
        ?: ""

    private fun materialObject(valueItem: Any): Any? = methodResult(valueItem, "getMaterial")
    ...
    private fun materialName(material: Any): String = methodResult(material, "getRegistryName")?.toRegistryMaterialId()
        ?: stringMethod(material, "getName")
        ?: ""

    private fun Any.toRegistryMaterialId(): String? = when (this) {
    ...
    private fun String.isRegistryLikeId(): Boolean = matches(Regex("[a-z0-9_.-]+:[a-z0-9_./-]+"))

    private fun stringMethod(target: Any, name: String): String? = (methodResult(target, name) as? String)?.takeUnless { it.isBlank() }

    private fun methodResult(target: Any, name: String): Any? = runCatching { target.javaClass.getMethod(name).invoke(target) }.getOrNull()
}
